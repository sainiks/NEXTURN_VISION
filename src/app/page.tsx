  ...
  return (
    <main className="min-h-screen">
      ...
      <section ref={heroRef} className="relative h-screen flex flex-col justify-center items-center overflow-hidden brutalist-border-b px-4">
        ...
        <div className="z-20 px-8 py-4 md:px-12 md:py-6 border-[3px] border-[var(--color-foreground)] bg-white/5 backdrop-blur-xl overflow-hidden flex justify-center items-center">
          <h1 className="text-[12vw] font-black tracking-tighter leading-none flex uppercase text-[var(--color-foreground)]">
        ...
        >
          <MagneticButton>Register Drive</MagneticButton>
          <Link href="/pipeline">
            <MagneticButton className="!bg-black !text-[var(--color-foreground)] hover:!bg-[var(--color-foreground)] hover:!text-black">
              View Pipeline
            </MagneticButton>
          </Link>
        </motion.div>
      ...
      <section ref={statsRef} className="py-32 px-8 border-b-[3px] border-[var(--color-foreground)]">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-4xl md:text-7xl font-bold uppercase mb-16">
            <TextScramble>Cell Statistics</TextScramble>
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 brutalist-border p-8 bg-[var(--color-foreground)] text-[var(--color-background)]">
            ...
            <div className="flex flex-col border-t-3 md:border-t-0 md:border-l-3 border-[var(--color-background)] pt-8 md:pt-0 md:pl-8">
      ...
      <section className="py-32 px-8 bg-[var(--color-background)] text-[var(--color-foreground)] overflow-hidden">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-4xl md:text-7xl font-bold uppercase mb-16 text-right">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-y-32">
            {[1, 2, 3, 4].map((item) => (
              <div 
                key={item}
                className="parallax-card inverted-hover brutalist-border p-8 cursor-pointer flex flex-col justify-between aspect-video relative group bg-[var(--color-background)]"
              >
                <div>
                  <h3 className="text-3xl font-black uppercase mb-2">Software Engineer</h3>
                  <p className="font-medium opacity-80 uppercase tracking-widest">B.Tech 2026</p>
                </div>
                <div className="flex justify-between items-end">
                  <span className="text-5xl font-black">0{item}</span>
                  <div className="w-12 h-12 brutalist-border rounded-full bg-[var(--color-accent)] transform scale-0 group-hover:scale-100 transition-transform origin-center ease-out duration-200" />
                </div>
              </div>
            ))}
          </div>
      ...
      {/* --- MASSIVE FOOTER --- */}
      <footer className="pt-32 pb-8 px-8 brutalist-border-t bg-[var(--color-foreground)] text-[var(--color-background)] overflow-hidden relative">
